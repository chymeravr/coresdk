...
void UVSphereBuilder::buildUnitSphere(Model *model,
                                      unsigned int nSubdivisionTimes) {
  ...
  unsigned int totalPhis =
      nPhiDivisions +
      extraRowsPerPole *
          2;  // will make 3 extra rows at poles to minimize distortion
  ...
  for (unsigned int iPhi = 0; iPhi <= totalPhis; iPhi++) {
    ...
    if (iPhi <= extraRowsPerPole) {
      ...
      uvy =
          (iPhi / (extraRowsPerPole + 1.0f)) * ((vMax - vMin) / nPhiDivisions) +
          vMin;
    } else if (iPhi >= totalPhis - extraRowsPerPole) {
      currentPhi = lowerPhi +
                   (totalPhis - (2.0f * extraRowsPerPole + 1)) * stepPhi +
                   (iPhi - (totalPhis - (extraRowsPerPole + 1.0f))) *
                       (stepPhi / (extraRowsPerPole + 1.0f));
      float vMaxMinusMin = vMax - vMin;
      uvy = vMaxMinusMin - vMaxMinusMin / nPhiDivisions +
            (iPhi - (totalPhis - extraRowsPerPole - vMaxMinusMin)) /
                (extraRowsPerPole + vMaxMinusMin) * vMaxMinusMin /
                nPhiDivisions +
            vMin;
    } else {
    ...
    for (unsigned int iTheta = 0; iTheta <= nThetaDivisions; iTheta++) {
      ...
      if (iTheta == nThetaDivisions) {
        currentTheta = lowerTheta;  // last vertex should be exactly same as
                                    // first vertex of the row
      }
      float cosphi = cosf(currentPhi);
      vertices.push_back(CL_Vec3(cosphi * cosf(currentTheta), sinf(currentPhi),
                                 cosphi * sinf(currentTheta)));
      CL_Vec2 uv;
      if (iPhi == 0 || iPhi == totalPhis) {
        uv.x = (uMax + uMin) / 2;  // Using the middle value of texture. Helpful
                                   // in getting just one seam as opposed to one
                                   // seam per triangle at poles
...
CL_Vec2 UVSphereBuilder::getUVMap(CL_Vec3 vertex) {
  ...
  if ((1 - y2) < epsilon) {
    ...
    if (sinTheta >
        1)  // checking if due to numerical error argument is not out of bound
      sinTheta = 1;
...
unsigned int UVSphereBuilder::getIndex(unsigned int iPhi, unsigned int iTheta,
                                       unsigned int nThetaDivisions) {
  return (nThetaDivisions + 1) * iPhi + iTheta;
}

CL_Vec3 UVSphereBuilder::getCartesianCoordinateFromThetaAndPhi(float phi,
                                                               float theta) {
  float y = sinf(phi);
