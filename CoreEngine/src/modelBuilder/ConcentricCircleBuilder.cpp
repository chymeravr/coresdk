...
namespace cl{
	void ConcentricCircleBuilder::buildConcentricCircle(Model *model, float rInner, float rOuter, unsigned int nDivisions){
		...
		CL_Vec3 previousInnerPoint = CL_Vec3(rInner*cosf(currentTheta), rInner*sinf(currentTheta), 0.0f);
		CL_Vec3 previousOuterPoint = CL_Vec3(rOuter*cosf(currentTheta), rOuter*sinf(currentTheta), 0.0f);
		...
		for (unsigned int i = 1; i <= nDivisions; i++){
			...
			CL_Vec3 currentInnerPoint = CL_Vec3(rInner*cosf(currentTheta), rInner*sinf(currentTheta), 0.0f);
			CL_Vec3 currentOuterPoint = CL_Vec3(rOuter*cosf(currentTheta), rOuter*sinf(currentTheta), 0.0f);
