...
namespace cl{
	ImagePNGLoader::ImagePNGLoader(ILogger *logger){
		this->logger = logger;
	}
...
//		logger->log(LOG_DEBUG, "Image Channels: " + std::to_string(channels));
	...
	void ImagePNGLoader::invertImage(unsigned char* data, unsigned int width, unsigned int height, unsigned int channels)
	{
