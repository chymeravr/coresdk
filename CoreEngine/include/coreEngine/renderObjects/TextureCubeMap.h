...
class TextureCubeMap : public Texture {
 public:
  ...
  const unsigned char *getTextureData(TEXTURE_CUBE_MAP_FACE face) {
  ...
  void setTextureDataSize(TEXTURE_CUBE_MAP_FACE face,
                          const unsigned int &dataSize) {
    this->dataSize[face] = dataSize;
  }
  const unsigned int &getTextureDataSize(TEXTURE_CUBE_MAP_FACE face) {
    return this->dataSize[face];
  }
  unsigned int getWidth(TEXTURE_CUBE_MAP_FACE face) { return width[face]; }
