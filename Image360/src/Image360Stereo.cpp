...
namespace cl {
    ...
    void Image360Stereo::initialize(TEXTURE_MAP_MODE mapMode,
        ...
        if (mapMode == EQUIRECTANGULAR_MAP_MODE) {
            ...
            ((MaterialDiffuseTexture *) this->stereoMaterial)->setDiffuseTexture(
                    this->stereoImageTexture);

            std::unique_ptr<TransformTreeCamera> transformTreeCameraUptr = transformTreeFactory->createTransformTreeCamera(
                    this->camera);
            this->camera->getComponentList().addComponent(std::move(transformTreeCameraUptr));

            std::unique_ptr<TransformTreeModel> leftTransformSphereUptr = transformTreeFactory->createTransformTreeModel(
            ...
            std::unique_ptr<TransformTreeModel> rightTransformSphereUptr = transformTreeFactory->createTransformTreeModel(
            ...
            transformTreeCamera = (TransformTreeCamera *) this->camera->getComponentList().getComponent(
                    "transformTree");
            transformTreeCamera->setLocalPosition(CL_Vec3(0.0f, 0.0f, 0.0f));
            transformTreeCamera->setLocalRotation(CL_Vec3(0.0f, 0.0f, 0.0f));

            TransformTreeModel *transformSphereLeft = (TransformTreeModel *) this->stereoImageContainer[LEFT]->getComponentList().getComponent(
                    "transformTree");
            ...
            TransformTreeModel *transformSphereRight = (TransformTreeModel *) this->stereoImageContainer[RIGHT]->getComponentList().getComponent(
                    "transformTree");
        ...
        notifyMeBackground = uiFactory->createPlanarBackground("notifyMe", scene.get(),
                                                               CL_Vec4(0.0, 0.0, 0.0, 0.7),
                                                               vec3_one, vec3_two, 3.0, 1.0);

        std::unique_ptr<FontStore> fontStore = uiFactory->createFontStore(scene.get(),
        ...
        std::unique_ptr<TextElement> notifyMeElement = uiFactory->createTextElement(
                "notifyMeElement", fontStore.get(), &textStyle, "Notify Me",
                vec3_one, vec3_two, scene.get());
        notifyMeBackground->addChild("child1", std::move(notifyMeElement));

        vec3_one = CL_Vec3(5.1, 0.0, -15.5);
        vec3_two = CL_Vec3(0.0, 0.0, 0.0);
        closeBackground = uiFactory->createPlanarBackground("closeMe", scene.get(),
                                                            CL_Vec4(0.0, 0.0, 0.0, 0.7), vec3_one,
                                                            vec3_two, 3.0, 1.0);

        vec3_one = CL_Vec3(-1.0, -0.2, 1.0);
        vec3_two = CL_Vec3(0.0, 0.0, 0.0);
        std::unique_ptr<TextElement> closeElement = uiFactory->createTextElement("closeElement",
                                                                                 fontStore.get(),
                                                                                 &textStyle,
                                                                                 "Close",
                                                                                 vec3_one, vec3_two,
        ...
        std::unique_ptr<IComponent> gazeDetectorNotifyMe = gazeDetectorFactory->createGazeDetectorBox(
                boxMessage, transformTreeCamera,
                transformNotifyMe, notifyMeListener.get(), gazeDetectorContainer.get(), vec3_one,
        ...
        TransformTreeModel *transformCloseMe = (TransformTreeModel *) closeMeModel->getComponentList().getComponent(
                "transformTree");
        ...
        vec3_one = CL_Vec3(0.0f, 0.0f, 0.0f);
        vec3_two = CL_Vec3(0.0f, 0.0f, -1.0f);
        std::unique_ptr<IComponent> gazeDetectorCloseMe = gazeDetectorFactory->createGazeDetectorBox(
                boxMessage2, transformTreeCamera,
                transformCloseMe, closeMeListener.get(), gazeDetectorContainer.get(), vec3_one,
                vec3_two, 3.0f, 1.0f, 0.00001f);
