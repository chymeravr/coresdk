...
namespace cl{
	...
	bool TextureCubeMapGL::initialize(){
		...
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, GL_RGB, width[TEXTURE_CUBE_MAP_FACE_RIGHT], height[TEXTURE_CUBE_MAP_FACE_RIGHT],
			0, GL_RGB, GL_UNSIGNED_BYTE, data[TEXTURE_CUBE_MAP_FACE_RIGHT].get());
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, GL_RGB, width[TEXTURE_CUBE_MAP_FACE_LEFT], height[TEXTURE_CUBE_MAP_FACE_LEFT],
			0, GL_RGB, GL_UNSIGNED_BYTE, data[TEXTURE_CUBE_MAP_FACE_LEFT].get());
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, GL_RGB, width[TEXTURE_CUBE_MAP_FACE_TOP], height[TEXTURE_CUBE_MAP_FACE_TOP],
			0, GL_RGB, GL_UNSIGNED_BYTE, data[TEXTURE_CUBE_MAP_FACE_TOP].get());
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, GL_RGB, width[TEXTURE_CUBE_MAP_FACE_BOTTOM], height[TEXTURE_CUBE_MAP_FACE_BOTTOM],
			0, GL_RGB, GL_UNSIGNED_BYTE, data[TEXTURE_CUBE_MAP_FACE_BOTTOM].get());
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, GL_RGB, width[TEXTURE_CUBE_MAP_FACE_BACK], height[TEXTURE_CUBE_MAP_FACE_BACK],
			0, GL_RGB, GL_UNSIGNED_BYTE, data[TEXTURE_CUBE_MAP_FACE_BACK].get());
		glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, GL_RGB, width[TEXTURE_CUBE_MAP_FACE_FRONT], height[TEXTURE_CUBE_MAP_FACE_FRONT],
			0, GL_RGB, GL_UNSIGNED_BYTE, data[TEXTURE_CUBE_MAP_FACE_FRONT].get());
	...
	void TextureCubeMapGL::rotateImageBy180(unsigned char* data, unsigned int width, unsigned int height, unsigned int dataSize){
		...
		//logger->log(LOG_INFO, "rotateImageBy180 texture channel count : " + std::to_string(channels));
