...
ShaderUniformColor *UniformColorFactoryGL::getShader(Scene *scene) {
  ...
  if (!(shader = (ShaderUniformColor *)scene->getFromScene(shaderId))) {
    // creating shader
    std::unique_ptr<ShaderUniformColor> shaderUptr(
        new ShaderUniformColorGL(shaderId, loggerFactory, scene));
...
MaterialUniformColor *UniformColorFactoryGL::getMaterial(
    ShaderUniformColor *shader, CL_Vec4 &color) {
  ...
  std::string materialId = "MaterialUniformColor" + cl::to_string(color[0]) +
                           cl::to_string(color[1]) + cl::to_string(color[2]) +
                           cl::to_string(color[3]);
  MaterialUniformColor *material = nullptr;
  if (!(material = (MaterialUniformColor *)scene->getFromScene(materialId))) {
    // creating material
    std::unique_ptr<MaterialUniformColor> materialUptr(
        new MaterialUniformColorGL(materialId, (ShaderUniformColorGL *)shader,
                                   loggerFactory, color));
