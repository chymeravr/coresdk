...
namespace cl
{
ShaderTextGLES2::ShaderTextGLES2(const std::string &sceneId, ILoggerFactory *loggerFactory,
				 Scene *scene) : ShaderText(sceneId, loggerFactory, scene)
{
    logger = loggerFactory->createLogger("glImplementation::ShaderTextGLES2: ");
}

IRenderable *ShaderTextGLES2::getRenderable()
{
    return this;
}

bool ShaderTextGLES2::initialize()
{
    ...
    {
	MaterialTextGLES2 *material = (MaterialTextGLES2 *)(*it);
...
void ShaderTextGLES2::draw()
{
    glUseProgram(programId);
    cameraGLContainer->draw();
}
void ShaderTextGLES2::deinitialize()
{
