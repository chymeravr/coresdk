...
namespace cl{
	ShaderTextGLES3::ShaderTextGLES3(const std::string &sceneId, ILoggerFactory *loggerFactory,
		Scene *scene) : ShaderText(sceneId, loggerFactory, scene){
		logger = loggerFactory->createLogger("glImplementation::ShaderTextGLES3: ");
	}
	
	IRenderable *ShaderTextGLES3::getRenderable()
	{
		return this;
	}

	bool ShaderTextGLES3::initialize()
	{
		...
		{
			MaterialTextGLES3 *material = (MaterialTextGLES3 *)(*it);
	...
	void ShaderTextGLES3::draw()
	{
		glUseProgram(programId);
		cameraGLContainer->draw();
	}
	void ShaderTextGLES3::deinitialize()
	{
