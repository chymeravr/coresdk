...
class DiffuseTextureGLFactory : public IDiffuseTextureFactory {
 public:
  ...
  std::unique_ptr<ShaderDiffuseTexture> createShader(const std::string &sceneId,
                                                     Scene *scene) {
    return std::unique_ptr<ShaderDiffuseTexture>(
        new ShaderDiffuseTextureGL(sceneId, loggerFactory, scene));
  }
  std::unique_ptr<MaterialDiffuseTexture> createMaterial(
      const std::string &sceneId, ShaderDiffuseTexture *shader) {
    return std::unique_ptr<MaterialDiffuseTexture>(new MaterialDiffuseTextureGL(
        sceneId, (ShaderDiffuseTextureGL *)shader, loggerFactory));
  }
