...
static bool isShuttingDown = false;
    ...
    static float startingAlpha = 1.0f;
    static float alphaDecrement = 0.05f;
...
bool TreasureHuntRenderer::IsFadedAway(){
    if (startingAlpha < 0.0f){
        return true;
    }
    return false;
};
...
void TreasureHuntRenderer::DrawFloor()
{
  ...
  glVertexAttrib3f(floor_normal_param_, 0.0f, 1.0f, 0.0f);
    if(isShuttingDown){
        glVertexAttrib4f(floor_color_param_, 0.0f, 0.3398f, 0.9023f, startingAlpha);
        startingAlpha -= alphaDecrement;
    }else {
          glVertexAttrib4f(floor_color_param_, 0.0f, 0.398f, 0.9023f, 1.0f);
    }
...
bool TreasureHuntRenderer::ObjectIsFound()
{
  ...
  case GVR_VIEWER_TYPE_CARDBOARD:
  {
      if((!isShuttingDown) && IsLookingAtObject()){
         isShuttingDown = true;
      }
